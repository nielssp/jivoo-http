...
class Routing extends Module
{
   ...
   */
    public function setRoot($route, $priority = 9)
    {
        ...
        if (isset($route['path'])) {
            ...
            } elseif ($route['path'] == $this->request->path) {
                $this->redirectPath(array(), $this->request->query);
            }
        } else {
   ...
   */
    private function getPathValidated($route)
    {
        ...
        } elseif (isset($this->paths[$routeString . '[*]'])) {
   ...
   */
    public function redirectPath(
        ...
        $this->triggerEvent('beforeRedirect', new RedirectEvent(
            $this,
            array('path' => $path, 'query' => $query, 'fragment' => $fragment),
            $moved
        ));
   ...
   */
    public function addRoute($pattern, $route, $priority = 5)
    {
        ...
        if ($method != 'ANY' && $method != $this->request->method) {
            $isMatch = false;
        } elseif ($patternc < count($path) and $pattern[$patternc - 1] != '**'
    ...
    public function followRoute($route, $status = null)
    {
        ...
        try {
            ...
            throw new InvalidResponseException(tr(
                'An invalid response was returned for: %1',
                $routeString
            ), null, $e);
        }
   ...
   */
    public function respond(Response $response, $status = null)
    {
        ...
        $this->logger->debug(tr(
            'Rendering response %1 (%2 %3)',
            get_class($response),
            $response->status,
            $response->type
        ));
      ...
      // TODO: to remove or not to remove PHP version from response?
        ...
        } elseif (function_exists('gzencode') and $this->request->acceptsEncoding('gzip')) {
